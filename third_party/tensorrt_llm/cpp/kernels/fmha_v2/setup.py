...
def _resolve_root_dir() -> Path:
    ...
    # Walk up from cwd and probe common layouts where fmha_v2 lives.
    for base in [cwd, *cwd.parents]:
        roots.append(base)
        roots.append(base / 'fmha_v2')
        roots.append(base / 'kernels' / 'fmha_v2')
        roots.append(base / 'cpp' / 'kernels' / 'fmha_v2')
        roots.append(base / 'tensorrt_llm' / 'cpp' / 'kernels' / 'fmha_v2')
        roots.append(base / 'third_party' / 'tensorrt_llm' / 'cpp' / 'kernels' / 'fmha_v2')
        roots.append(base / 'vllm' / 'third_party' / 'tensorrt_llm' / 'cpp' / 'kernels' / 'fmha_v2')

    for root in roots:
        if (root / 'src' / 'fused_multihead_attention_kernel.h').exists():
            return root
    return cwd
...
generate_cu_trtllm = os.environ.get('GENERATE_CU_TRTLLM',
                                    'False').lower() == 'true'
use_cubin_header_sm80 = os.environ.get('VLLM_FMHA_USE_CUBIN_HEADER_SM80',
                                       '0') == '1'
...
def use_cubin_header(sm,
    ...
    if 'e4m3' in dtype and output_dtype in ['bf16', 'fp16']:
        return False
    if use_cubin_header_sm80 and sm == 80:
        return True
...
def generate_files(specs_names):
    ...
    cmd = [
        ...
        '--std=c++17',
        '-o',
        os.path.join(ROOT_DIR, 'bin', 'print_traits.exe'),
        os.path.join(ROOT_DIR, 'generated', 'print_kernel_traits.cu'),
    ]
    if 'CUDA_PATH' in os.environ:
        cmd[0] = os.path.join(os.environ['CUDA_PATH'], 'bin', 'nvcc')
    ...
    process = subprocess.Popen(os.path.join(ROOT_DIR, 'bin',
                                            'print_traits.exe'),
                               stdin=subprocess.PIPE,
                               stdout=subprocess.PIPE,
                               cwd=ROOT_DIR)
