...
@click.option(
    ...
    default=False,
    help=help_info_with_stability_tag(
        "Enable XQA JIT for TensorRT backend (sets TRTLLM_ENABLE_XQA_JIT=1).",
        "prototype"),
)
@click.option(
    "--disable-xqa-jit",
    ...
    default=False,
    help=help_info_with_stability_tag(
        "Disable XQA JIT for TensorRT backend (sets TRTLLM_ENABLE_XQA_JIT=0).",
        "prototype"),
)
@click.option(
    "--custom_module_dirs",
...
def serve(
        ...
        enable_xqa_jit: bool, disable_xqa_jit: bool,
    ...
    if enable_xqa_jit and disable_xqa_jit:
        raise click.UsageError(
            "Cannot set both --enable-xqa-jit and --disable-xqa-jit.")
    if enable_xqa_jit or disable_xqa_jit:
        os.environ["TRTLLM_ENABLE_XQA_JIT"] = "1" if enable_xqa_jit else "0"
    elif backend == "tensorrt" and "TRTLLM_ENABLE_XQA_JIT" not in os.environ:
        # Default to precompiled XQA for TensorRT backend.
